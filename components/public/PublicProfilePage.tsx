...
export function PublicProfilePage({ user }: PublicProfilePageProps) {
  ...
  const [currentBackgroundAnimation, setCurrentBackgroundAnimation] =
    useState<string>("");
  ...
  // Initialize theme from user's saved preferences
  useEffect(() => {
    if (!isMounted) return;

    // Set color theme - use new colorTheme field if available, otherwise fallback to old theme
    let colorThemeId = user.colorTheme;
    if (!colorThemeId && user.theme) {
      // Migration: try to find matching color theme by ID or name
      const matchingTheme = colorThemes.find(
        (theme) => theme.id === user.theme || theme.name === user.theme
      );
      colorThemeId = matchingTheme?.id || colorThemes[0].id;
    } else if (!colorThemeId) {
      colorThemeId = colorThemes[0].id;
    }
    setCurrentColorTheme(colorThemeId);

    // Set background animation
    let animationId = user.backgroundAnimation;
    if (!animationId) {
      // Use random animation if none is set
      const randomIndex = Math.floor(
        Math.random() * backgroundAnimations.length
      );
      animationId = backgroundAnimations[randomIndex].id;
    }
    setCurrentBackgroundAnimation(animationId);

    // Set system theme based on color theme mode
    const selectedColorTheme = colorThemes.find((ct) => ct.id === colorThemeId);
    if (selectedColorTheme) {
      setSystemTheme(selectedColorTheme.mode);
    }
  }, [
    user.colorTheme,
    user.backgroundAnimation,
    user.theme,
    setSystemTheme,
    isMounted,
  ]);
  ...
  const theme =
    colorThemes.find((ct) => ct.id === currentColorTheme) || colorThemes[0];
  const backgroundAnimation =
    backgroundAnimations.find((ba) => ba.id === currentBackgroundAnimation) ||
...
END:VCARD`;
  ...
  const selectThemeAndAnimation = async (
    colorThemeId: string,
    ...
    if (backgroundAnimationId) {
      setCurrentBackgroundAnimation(backgroundAnimationId);
    }

    // Override system theme with the selected color theme's mode
    if (selectedColorTheme) {
      setSystemTheme(selectedColorTheme.mode);
    }

    // Save theme and animation to database if this is the user's own profile
    if (isOwnProfile) {
      try {
        const response = await fetch("/api/profile/theme-and-animation", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (response.ok) {
          ...
          toast.success(
            `💀 THEME BRUTALLY SAVED! ${selectedColorTheme?.name} + ${backgroundAnimations.find((ba) => ba.id === (backgroundAnimationId || result.backgroundAnimation))?.name}! 💀`
          );
        } else {
          throw new Error("Failed to save theme and animation");
        }
      } catch (error) {
        console.error("Failed to save theme and animation:", error);
        toast.error("Failed to save theme - THE BRUTALITY IS TOO STRONG!");
        // Revert on error
        const fallbackColorTheme =
          user.colorTheme ||
          (user.theme
            ? colorThemes.find((ct) => ct.id === user.theme)?.id
            : colorThemes[0].id) ||
          colorThemes[0].id;
        const fallbackAnimation =
          user.backgroundAnimation || backgroundAnimations[0].id;
        setCurrentColorTheme(fallbackColorTheme);
        setCurrentBackgroundAnimation(fallbackAnimation);
      }
    } else {
      toast.success(
        `💀 THEME BRUTALLY CHANGED! ${selectedColorTheme?.name}! 💀`
      );
    }
  };
  ...
  return (
    <>
      {!isMounted ? (
        // Render a simple loading state to prevent hydration mismatches
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-600 to-pink-600">
          <div className="text-white text-2xl font-black">
            💀 LOADING BRUTAL PROFILE... 💀
          </div>
        </div>
      ) : (
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              backgroundImage: getAnimationStyle(backgroundAnimation.id),
              backgroundSize: getAnimationSize(backgroundAnimation.id),
              animation: getAnimationKeyframes(backgroundAnimation.id),
            }}
          />

          {/* Additional animated overlay for complex patterns */}
          {isComplexAnimation(backgroundAnimation.id) && (
            <div
              className="absolute inset-0 opacity-5"
              style={{
                backgroundImage: getSecondaryAnimationStyle(
                  backgroundAnimation.id
                ),
                backgroundSize: getSecondaryAnimationSize(
                  backgroundAnimation.id
                ),
                animation: getSecondaryAnimationKeyframes(
                  backgroundAnimation.id
                ),
              }}
            />
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div
                        className={`text-lg font-bold ${theme.textSecondary}`}
                      >
                        @{user.username}
                      </div>
                ...
                >
                  <div className="space-y-3">
                    <motion.div
                      ...
                      onClick={() => {
                        if (!isMounted) return;
                        window.open("https://lyntbrutt.com", "_blank");
                      }}
                    >
                      <GlitchText
                        className={`text-2xl font-black ${theme.text}`}
                    ...
                    <p
                      className={`text-lg font-semibold ${theme.textSecondary}`}
                    >
          ...
          <ThemePicker
            ...
            currentBackgroundAnimation={currentBackgroundAnimation}
            onThemeAndAnimationSelect={selectThemeAndAnimation}
            theme={theme}
